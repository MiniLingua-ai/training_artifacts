...
export SINGULARITY_BIND=/scratch

TOKENIZER_DIR=/scratch/cs/small_lm/ConvertedTokenizer  # <----- Change this to your needs
...
if [[ $iter_segment =~ iter_([0-9]+) ]]; then
    ...
    #CHECKPOINT_PATH="/scratch/project_462000756/checkpoints/1B_bs_1024_lr_0005_warmup_6000_start_44000" #$LOAD_DIR
    ...
    singularity exec $CONTAINER /bin/bash -c "
      ...
      python3 /scratch/cs/small_lm/Megatron-LM/tools/checkpoint/convert.py\
        --megatron-path /scratch/cs/small_lm/Megatron-LM \
        --model-type GPT \
        --loader mcore \
        --saver llama_mistral \
